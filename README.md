...
- **Zoo and aquarium records are excluded from the map and the notable feed.** Captive exhibit animals are rarely flagged as captive by observers, so an exotic species logged inside the Bronx Zoo reptile house can pass as "research grade." Because there is no reliable way to separate a wild squirrel on zoo grounds from an animal in an enclosure, every observation whose location names a zoo or aquarium is dropped from the map and the notable ranking. (The full census counts still reflect raw iNaturalist totals.)
